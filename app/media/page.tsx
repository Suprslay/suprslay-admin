...
export default async function MediaManager() {
  ...
  return (
    <div className="p-8">
      ...
      <h2 className="text-xl font-semibold mt-6 mb-4">Audio</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {audio.map((blob) => (
          <div key={blob.pathname} className="border p-4 rounded">
            <audio src={blob.url} className="w-full mb-2" controls />
            <p className="truncate">{blob.pathname}</p>
            <p className="text-sm text-gray-500">{new Date(blob.uploadedAt).toLocaleString()}</p>
            <form
              action={async () => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {images.map((blob) => (
          <div key={blob.pathname} className="border p-4 rounded">
            <img src={blob.url || "/placeholder.svg"} className="w-full h-48 object-cover mb-2" alt={blob.pathname} />
            <p className="truncate">{blob.pathname}</p>
            <p className="text-sm text-gray-500">{new Date(blob.uploadedAt).toLocaleString()}</p>
            <form
              action={async () => {
